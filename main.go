...
var (
	...
	commands = []*discordgo.ApplicationCommand{
		{
			Name:        "move",
			Description: "move all members in source voice channel to destination.",
			DescriptionLocalizations: &map[discordgo.Locale]string{
				discordgo.Japanese: "コマンドで指定したVCにいるメンバー全員を別VCに移動させます。",
			},
	...
	commandHandlers = map[string]func(s *discordgo.Session, i *discordgo.InteractionCreate){
		"move": func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			guild, err := s.Guild(i.GuildID)
			if err != nil {
				s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
					Type: discordgo.InteractionResponseChannelMessageWithSource,
					Data: &discordgo.InteractionResponseData{
						Content: "this command is only available in guild.",
					},
				})
				return
			}
			...
			var source, destination *discordgo.Channel

			if opt, ok := optionMap["src"]; ok {
				source = opt.ChannelValue(s)
			} else if voiceState, err := s.State.VoiceState(i.GuildID, i.User.ID); err != nil {
				s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
					Type: discordgo.InteractionResponseChannelMessageWithSource,
					Data: &discordgo.InteractionResponseData{
						Content: "error!",
				...
				if vc, err := s.State.Channel(voiceState.ChannelID); err != nil {
					s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
						Type: discordgo.InteractionResponseChannelMessageWithSource,
						Data: &discordgo.InteractionResponseData{
							Content: "error!",
			...
			if opt, ok := optionMap["dest"]; ok {
				destination = opt.ChannelValue(s)
			} else {
				s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
					Type: discordgo.InteractionResponseChannelMessageWithSource,
					Data: &discordgo.InteractionResponseData{
			...
			isBothSrcAndDestGuildVoice :=
				source.Type == discordgo.ChannelTypeGuildVoice &&
					destination.Type == discordgo.ChannelTypeGuildVoice

			if !isBothSrcAndDestGuildVoice {
				s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
					Type: discordgo.InteractionResponseChannelMessageWithSource,
					Data: &discordgo.InteractionResponseData{
						Content: "error 2!",
			...
			for _, v := range guild.VoiceStates {
				fmt.Println(v.ChannelID == source.ID)
				if v.ChannelID == source.ID {
					s.GuildMemberMove(i.GuildID, v.UserID, &destination.ID)
				}
			}
			...
			s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
				Type: discordgo.InteractionResponseChannelMessageWithSource,
				Data: &discordgo.InteractionResponseData{
					Content: "moved members in vc to another",
				},
			})
		},
